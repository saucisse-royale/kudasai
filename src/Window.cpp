...
namespace kds {
  ...
  void Window::init(WindowConfig const& windowConfig, const std::function<void(Drawer&)>& renderCallback)
  {
  ...
  void Window::init(ContextConfig const& contextConfig, const std::function<void(Drawer&)>& renderCallback)
  {
    closed = false;
    width = contextConfig.windowConfig.width;
    height = contextConfig.windowConfig.height;
  ...
  {
    glfwPollEvents();
    std::vector<Input> inputs = std::move(this->inputs);
    this->inputs.clear();
    return inputs;
  }
